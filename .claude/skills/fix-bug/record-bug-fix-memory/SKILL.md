...
### `packages/vue-element-cui-nuxt` 的 Vercel 云函数依赖缺失事故（`trace: false` 的下游灾难，2026-04）

**问题现象**

- 文档站部署到 Vercel 后，所有 SSR 页面返回运行时错误，云函数无法启动；本地检查 `packages/vue-element-cui-nuxt/.vercel/output/functions/__fallback.func` 目录，发现缺失 `node_modules` 和 `package.json`，只有 `chunks/`、`index.mjs` 和 `.vc-config.json`。

**实际根因**

- `nuxt.config.ts` 中 `nitro.externals.trace = false` 关闭了 `@vercel/nft`（Node File Trace）。该组件负责在 Nitro 构建收尾阶段扫描 server bundle 的所有 `require()`/`import` 调用，追踪到 `node_modules` 中实际需要的文件，并将其复制到 `__fallback.func/node_modules/` 目录、生成配套的 `package.json`。`trace = false` 直接跳过了这整个过程，导致产物自始至终不含任何运行时依赖。

**关键误导点**

- `trace: false` 是之前为解决 Windows + pnpm workspace 下 `@vercel/nft` 卡死问题而添加的，被当作"安全的性能优化"。由于该配置不影响本地 `nuxt dev` 和客户端构建，且构建日志正常显示 `Build complete!`，开发者不容易意识到 server 产物已经残缺。对比参考项目 `D:\code\ruan-cat\notes\docs\my-pull-requests\nuxt.config.ts`（无 `externals` 配置，trace 默认启用），其产物正常包含 `node_modules`。

**有效修复**

- 将 `nitro.externals.trace = false` 替换为 `nitro.externals.inline = [/.*/]`。这让 Rollup 在打包阶段把全部依赖内联进 server JS chunk（主 chunk `nitro.mjs` 约 3MB），云函数完全自包含，既不需要 `node_modules`，也不需要 `@vercel/nft` 做文件追踪。构建耗时从卡死恢复到约 1 分 46 秒。

**验证方式**

- `pnpm run build:docs:nuxt-doc` 成功退出（exit code 0）；`__fallback.func/` 包含 `package.json`（`"dependencies": {}`，无外部依赖）；`chunks/` 目录总计约 13MB，所有运行时依赖（Vue SSR、Nitro、shiki 语法高亮等）均已打包进 `.mjs` 文件；构建日志出现 `Build complete!` 和 `You can deploy this build using npx vercel deploy --prebuilt`。

**后续约束**

- **永远不要使用 `nitro.externals.trace = false`**。它是一个破坏性的配置，会让 Vercel/Netlify 等 serverless 平台的云函数在运行时找不到依赖。
- 需要绕开 Windows + pnpm workspace 的 `@vercel/nft` 卡死时，用 `externals.inline = [/.*/]` 代替。
- 对 `nitro.externals` 的任何改动，必须在本地构建后检查 `__fallback.func/` 目录结构，确认产物完整性（要么有 `node_modules` + `package.json`，要么 `package.json` 的 `dependencies` 为空且 chunks 包含内联依赖）。
- 排查 Vercel 部署失败时，首个检查点是 `__fallback.func/` 目录结构，而不是云端日志。
