...
### `packages/vue-element-cui` 的 CuiDialog 弹层遮罩完全失效事故（Element Plus appendToBody 默认值变更 + CSS 堆叠上下文，2026-04）

**问题现象**

- 文档站（dev 和生产环境均复现）点击「打开确认弹层」按钮后，对话框弹出但**没有半透明遮罩层覆盖页面**——背后的 header、sidebar、正文内容全部清晰可见可点击，对话框只是浮在页面上方，视觉层级完全错乱。

**实际根因（两层叠加）**

- **第一层（Teleport 被禁用）**：Element Plus 2.13.x 将 dialog 的 `appendToBody` 属性类型改为 `Boolean`（无显式默认值），Vue 3 中 Boolean prop 未传时默认 `false`。源码中 `ElTeleport` 的 `disabled` 计算为 `!appendToBody`，即 `!false = true`，导致 `<Teleport to="body">` 被禁用。overlay 不再挂载到 `<body>`，而是留在组件所在的 DOM 层级中。
- **第二层（CSS 堆叠上下文困住 overlay）**：文档站的 demo 容器 `.cui-demo-container` 设有 `isolation: isolate`（用于样式隔离），创建了新的堆叠上下文。overlay 虽然是 `position: fixed` 且 `z-index: 2002`，但它的 z-index 只在 `.cui-demo-container` 的堆叠上下文内生效，无法覆盖页面上层的 header（`z-index: 40`）、sidebar（`z-index: 30`）等元素。

**关键误导点**

- **误导 1**：最初怀疑是 `nuxt.config.ts` 中 `vite.ssr.noExternal` 的依赖外部化配置过激导致 CSS 丢失。实际检查生产 CSS 文件（367KB）后确认 Element Plus 全部样式类（`.el-dialog`、`.el-overlay`、`.el-button`）、CSS 变量（`--el-color-primary`、`--el-overlay-color-lighter`）均存在且完整。
- **误导 2**：怀疑 Tailwind CSS v4 的 `@layer base` preflight 重置覆盖了 Element Plus 样式。实际上 Element Plus CSS 是非分层（unlayered）样式，在 CSS 级联中始终优先于 `@layer base` 中的规则。
- **误导 3**：通过 `getComputedStyle` 检查 overlay 时，所有属性看起来完全正确（`background-color: rgba(0,0,0,0.5)`、`position: fixed`、`inset: 0px`、`z-index: 2002`），但页面视觉上没有遮罩效果。**关键转折点**是检查 `overlay.parentElement` 发现其父节点是 `DIV.space-y-4`（demo 组件内部）而不是 `BODY`，然后追溯祖先链找到了 `isolation: isolate`。

**有效修复**

- 在 `CuiDialog` 组件的 `<el-dialog>` 上添加 `append-to-body` 属性，强制启用 Teleport 将 overlay 挂载到 `<body>` 层级，绕过 `.cui-demo-container` 的堆叠上下文限制。

**验证方式**

- `document.querySelector('.el-overlay').parentElement === document.body` 返回 `true`，确认 overlay 已挂载到 body。
- `document.elementFromPoint()` 在页面各位置（header、sidebar、content、corner）均返回 `el-overlay-dialog`，确认遮罩覆盖全页面。
- 292 个组件库测试全部通过，无回归。

**后续约束**

- **升级 Element Plus 后必须检查弹层组件的 Teleport 行为**。Element Plus 的 `appendToBody`/`teleported` 默认值在不同大版本间有变化（旧版 `teleported` 默认 `true`，新版 `appendToBody` 默认 `false`），升级后如果弹层不显示遮罩，优先检查此项。
- **使用 `isolation: isolate` 创建堆叠上下文时，必须同时确保内部的弹层组件开启了 Teleport**，否则 overlay 的 `z-index` 会被困在该上下文内。
- **排查「overlay 样式正确但视觉不生效」时，首个检查点是 `overlay.parentElement` 是否为 `body`**，而不是检查 CSS 属性值。CSS 属性全部正确但 overlay 不可见的最常见原因是 Teleport 失效 + 堆叠上下文。
